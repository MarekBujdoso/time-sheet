...
const InterruptionTime = (props: InterruptionTimeCompProps) => {
  ...
  const interruption: InterruptionTimeProps = { id, type, startTime, endTime, time: new Decimal(differenceInMinutes(endTime, startTime) / 60) }
  ...
  return (
    <div className="col-span-2 flex items-center space-x-2 justify-between">
      <Label className={`w-[100px] ${isDisabled ? 'cursor-not-allowed opacity-50' : ''}`} htmlFor="doctorsLeave">{interruptionTitle}</Label>
      <div className="flex items-center space-x-1">
        <Input className={`w-[100px] ${isInvalid ? 'border-red-500' : ''} ${isDisabled ? 'cursor-not-allowed' : ''}`}
      ...
      <div  className="flex items-center space-x-1">
        ...
        <Button disabled={isDisabled} variant={'outline'} type="button" onClick={() => remove(id)}><X /></Button> 
      </div>
    </div>
  )
}
